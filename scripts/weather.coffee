...
#
# Configuration:
#   HUBOT_WEATHER_API_URL - Optional openweathermap.org API endpoint to use. Defaults to v2.5.
#   HUBOT_WEATHER_UNITS - Temperature units to use. 'metric' or 'imperial'. Defaults to imperial.
#
# Commands:
...
module.exports = (robot) ->
  robot.hear /weather in (\w+)/i, (msg) ->
    city = msg.match[1]
    query = "units=#{process.env.HUBOT_WEATHER_UNITS}&q=#{encodeURIComponent(city)}"
